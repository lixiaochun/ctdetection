...
public class WebObject
{
	...
	private List<JSONArray> m_listJson = new ArrayList<JSONArray>();	//Json结果
	...
	public WebObject(Context context, WebPar par,Handler handler,int what)
	{
		...
		m_httpSocket = new HttpSocket(m_webPar,m_strPath);
	...
	public JSONArray runningTask(JSONArray jsonArr)
	{
		...
		for (int i = 0; i < m_webPar.m_nCount && m_curState == TestState.eTesting; i++)
		{
			...
			//保存记录
			jsonRec = m_aidInfo.getJsonVal(null);
			jsonRec = webRet.getJsonVal(jsonRec);
			jsonArr.put(jsonRec);
			m_listJson.add(jsonRec);
			//---------------------------------------
