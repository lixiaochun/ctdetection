...
public abstract class AbsMainFragment extends Fragment
{
	...
	protected boolean m_bSingleFrag = false; // 是否是单独界面
...
class FragmentTabHost extends TabHost implements TabHost.OnTabChangeListener
{
	...
	public void addTab(TabHost.TabSpec tabSpec, Class<?> clss, Bundle args)
	{
		...
		if (mAttached)
		{
			...
			{
				FragmentTransaction ft = mFragmentManager.beginTransaction().setCustomAnimations(android.R.anim.fade_in, android.R.anim.fade_out);
		...
		for (int i = 0; i < mTabs.size(); i++)
		{
			...
			if (tab.fragment != null)
			{
				...
				else
				{
					...
					if (ft == null)
					{
						ft = mFragmentManager.beginTransaction().setCustomAnimations(android.R.anim.fade_in, android.R.anim.fade_out);
					}
		...
		if (hidden)
			for (int i = 0; i < mTabs.size(); i++)
			{
				if (mTabs.get(i).fragment != null)
					mTabs.get(i).fragment.onHiddenChanged(hidden);
			}
		else if (mLastTab.fragment != null)
			mLastTab.fragment.onHiddenChanged(hidden);
	}
}